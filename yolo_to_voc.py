...
## path root folder
ROOT = '/content/YOLOX/datasets/VOCdevkit'


## converts coco into xml 
def xml_transform(root, classes):  
    class_path  = join(root)
    ids = list()
    ...
    imgpath = join(root, '%s.png')#jpg')
    
    # os.makedirs(join(root, 'outputs'), exist_ok=True)
    outpath = join(root, '%s.xml')
    ...
    for i in range(len(ids)):
        ...
        height, width, channels = 768, 768, 3  # img.shape # pega tamanhos e canais das images
        ...
        # bb values were not normalized before so
        width, height = 1, 1  # for S2SHIPS
        ...
        if os.path.exists(target):
            ...
            for i in range(len(label_norm)):
                ...
                node_name.text = 'ship' #classes[new_label[0]]
